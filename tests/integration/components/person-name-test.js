...
module('Integration | Component | person-name', function(hooks) {
  ...
  test('add button functionality', async function (assert) {
    this.set('addName', (firstName, lastName) => {
      assert.ok('Function should be called');
      assert.equal(firstName, 'tony', 'first name should be sent out from person-name');
      assert.equal(lastName, 'stark', 'last name should be sent out from person-name');
    })
